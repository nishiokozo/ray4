...
//------------------------------------------------------------------------------
bool raycast( Surface& sur, const vect3& P, const vect3& I, double max_t, double valRefractive )
//------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------
Surface raygun( vect3& P, const vect3& I )
//------------------------------------------------------------------------------
{
	Surface sur;
	...
	for ( int i = 0 ; i < m_cntPlate ; i++ )
	{
		...
		if ( f > 0 )
		{
			...
			if ( sur.t >= t && t >= 0 )
			{
				...
				{
					sur.C = obj.C;
					if ( sur.Q.z > 0 ) sur.C = vect3(1,0,0);
					if ( sur.Q.z <=0 ) sur.C = vect3(0,0,1);
					if ( sur.Q.x >=0 ) sur.C += vect3(0,0.5,0);
					if ( sur.Q.x < 0 ) sur.C += vect3(0,1,0);
				}
				else
				{
					sur.C = obj.C * 0.5;
					if ( sur.Q.z > 0 ) sur.C = vect3(0.5,0,0);
					if ( sur.Q.z <=0 ) sur.C = vect3(0,0,0.5);
					if ( sur.Q.x >=0 ) sur.C += vect3(0,0.25,0);
					if ( sur.Q.x < 0 ) sur.C += vect3(0,0.5,0);
				}
	#else
				if (   (fmod(sur.Q.x+10e3, 1.0) < 0.5 && fmod(sur.Q.z+10e3, 1.0) < 0.5 )
